...
def read_sheet(xl: pd.ExcelFile, sheet_name: str) -> list[dict]:
    ...
    for _, row in df.iterrows():
        ...
        ref_num = parse_int(row.get('reference number', row.get('reference_number', 0)), 0)

        questions.append({
            'id':                   str(uuid.uuid4()),
            'reference_number':     ref_num if ref_num > 0 else None,
            'text':                 text_en,
...
def generate_sql(all_questions: list[dict]) -> str:
    ...
    lines = [
        ...
        "",
        "-- Ensure driver_categories and reference_number columns exist",
